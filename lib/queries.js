...
// Open jobs shown on the public careers page
export async function getOpenJobs() {
  return await sql`
    SELECT id, role_name, location, created_at
    FROM jobs
...
// One job, only if it's still visible on the website
export async function getOpenJobById(id) {
  if (!/^\d+$/.test(String(id))) return null
  const [job] = await sql`
    SELECT id, role_name, description, location, show_on_website, created_at
    FROM jobs
    WHERE id = ${id} AND show_on_website = true
  `
  return job || null
}

// Shared "About Company" blurb shown on every job posting
export async function getAboutCompany() {
  const [row] = await sql`SELECT about_company FROM careers_settings WHERE id = 1`
  return row?.about_company || ''
}